...
describe("direct desktop Google authorization", () => {
  it("accepts a desktop client ID and never treats a relay address as a client", () => {
    ...
    expect(new GoogleAuthManager(clientId, http, new Secrets()).isAvailable()).toBe(true);
    expect(new GoogleAuthManager("https://relay.example", http, new Secrets()).isAvailable()).toBe(false);
  });
  ...
  it("receives approval on loopback and exchanges and refreshes only with Google", async () => {
    ...
    const auth = new GoogleAuthManager(clientId, http, secrets);
    ...
    const reloaded = new GoogleAuthManager(clientId, http, secrets);
